export async function loadReverb(audioFile: string, context: AudioContext) {
...
export function buildReverb(context: AudioContext) {
  const convolver = context.createConvolver()

  // Reverb
  const reverbTime = 0.2 // the duration of the reverb effect, in seconds
