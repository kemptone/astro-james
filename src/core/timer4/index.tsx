...
const InnerCore = ({ Sounds }: { Sounds: AudioThing[] }) => {
  ...
  const startFan = useCallback(() => {
    if (
      ...
      const [rotations_speedup, rotations_slowdown, rotations_runtime] = [
      ...
      setBodyStyleProp('--rotations_speedup', 360 * rotations_speedup + 'deg')
  ...
  }, [e_runTime.current, e_slowDown.current, e_speedUp.current, rate])
        ...
        useAudioLoop(s, context, ({ source }) => {
          if (source && context) {
            ...
            source.playbackRate.setValueCurveAtTime(
              [0, 0.1, 0.3, 0.6, 1].map(
                i => i * s.initialPlaybackRate * (rate * audioRate)
              ),
              context.currentTime,
              rampUp || 1
            )
            ...
            source.playbackRate.setValueCurveAtTime(
              [1, 0.9, 0.8, 0.7, 0.5, 0.3, 0.1, 0].map(
                i => i * s.initialPlaybackRate * (rate * audioRate)
              ),
            ...
            source.stop(context.currentTime + rampUp + run + slow + 2)
