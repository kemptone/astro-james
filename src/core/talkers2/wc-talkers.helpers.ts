...
import {fetchTalkers2Audio} from './audio-access'
...
export const playMeme = async ({
  ...
  const response = await fetchTalkers2Audio('/api/get_meme', {
    ...
    body: JSON.stringify({audio, audioFingerprint}),
  })
  if (response.status === 409 && typeof document !== 'undefined') {
    document.dispatchEvent(
      new CustomEvent('meme_audio_changed', {detail: {audio}})
    )
  }
